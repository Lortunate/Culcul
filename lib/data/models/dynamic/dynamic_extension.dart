...
extension DynamicItemExtension on DynamicItem {
  ...
  DynamicLinkCard? get linkCard => _mapLinkCard(modules.moduleDynamic.major);

  String? get commentId => basic?.commentIdStr;
  int? get commentType => basic?.commentType;

  DynamicAdditional? get additional => _mapAdditional(modules.moduleDynamic.additional);

  /// Creates a copy of the DynamicItem with updated like status and count.
  /// Handles the deep nesting of the moduleStat structure.
  ...
  // Private helpers (copied from DynamicMapper)
  DynamicVideoContent? _mapVideoContent(ModuleMajor? major) {
  ...
  DynamicLinkCard? _mapLinkCard(ModuleMajor? major) {
    if (major == null) return null;
