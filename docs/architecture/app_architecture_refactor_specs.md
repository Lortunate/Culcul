...
Migration note: current `lib/ui/` remains valid only during migration. Do not create a
parallel `shared/` component for an existing `ui/` component unless the old component is
moved or deleted in the same phase.
...
- Publish-dynamic page-only chrome and picker UI stay private to
  ...
  `PublishDynamicImageGrid`, `EmojiPicker`, `TopicPicker`, or their one-owner widget
...
- Route-level page factories are archived when their only behavior is returning a single
  page constructor that the app router can own directly.
...
- Dynamic post action sheets stay with their owning header/card when they have a single
  consumer. Do not restore `showDynamicPostActions`, `DynamicPostAction`, or
...
- Video action bottom-sheet widgets are home-owned when their only entrypoint is
  ...
  Keep the home sheet drag handle inline with `_HomeVideoActionsBottomSheet.build`;
  do not restore a one-use `_buildDragHandle` method for a fixed visual element.
...
- Auth QR login polling state is view-owned while it has a single consumer. Keep QR URL
  ...
  refresh retry, and post-success auth refresh private to `QrLoginView`; do not restore
  `auth_qr_login_view_model.dart` or public QR-login provider/state shells.
...
- Notification page-only stream state is page-owned when one page consumes it. Keep
  ...
  and system-feed empty fallback private to `NotificationListPage`. Do not restore
...
- Profile action-grid action builders are archived when they only assemble one page's
  icon-label callback list; keep the concrete action list inline with `ProfilePage`.
...
- User profile app-bar menu helpers are archived when the menu is opened only by
  ...
  more button owner and do not restore `_showMoreMenu`.
...
- Profile relation route page wrappers are archived when they only select one relation
  ...
  builders public, but keep the concrete relation page private beside profile navigation.
...
- Dynamic forward wrappers are archived when only `DynamicContentWidget` recurses into
  ...
  and recursive content rendering private beside the owning content widget.
...
- Dynamic additional/content card wrappers are archived when only `DynamicContentWidget`
  ...
  owned inline by `_buildAdditional`; do not restore `_DynamicVoteCard`.
- Dynamic loading skeleton wrappers are archived when they only serve one feed owner.
  Keep user dynamic feed loading shimmer markup inline with `_DynamicSkeleton.build`
  instead of exporting a shared `DynamicSkeleton` widget or retaining one-call
  header/content/footer helpers without a second consumer.
...
- Search history persistence state is page-owned while only `SearchPage` reads and
  ...
  normalization helpers, but do not restore `search_history_application_providers.dart`
  or public `SearchHistory` provider shells.
...
- `AppTag` color fallbacks should stay inline with `AppTag.build`; do not restore
  one-call `_backgroundColor` or `_foregroundColor` methods that only wrap constructor
  overrides and `ColorScheme` defaults.
...
- `AppCardContainer` decoration construction should stay inline with the
  `ValueListenableBuilder` branch that owns the current `PerformancePolicy`; keep the
  `_buildShadow` policy helper, but do not restore a one-call `_buildDecoration`
  wrapper around `BoxDecoration`.
...
- Article detail stats chips should stay inline with `_StatsRow.build`; do not restore a
  one-call `_StatChip` widget that only pairs an icon and formatted count for the
  favorite/like row.
...
- Shared search-bar icon helpers are archived when they only wrap the same static search
  ...
  while shared decoration/text-style helpers may remain when they prevent styling drift.
- The read-only `AppSearchBar` branch should stay inline with `AppSearchBar.build` so
  ...
  helper wrapper. Preserve the `controller != null` editable `TextField` split, height,
  shared decoration, hint overflow, and disabled `onTap == null` behavior.
...
- Home video actions rows are owned inline by `_HomeVideoActionsBottomSheet.build`.
  Do not restore `_ActionItem`; the sheet may keep the two action rows as direct
  `InkWell` blocks with the same padding, icon, and bodyLarge typography.
...
- Single-use gesture feedback widgets are archived when they only serve
  ...
  private beside the gesture owner instead of exporting player-control wrappers.
...
- Single-owner comment image wrappers are archived when only `CommentItemWidget` uses
  ...
  tags, and preview opening private beside the comment content owner.
...
- Player overlay panel APIs expose only rendered behavior. Do not keep
  ...
  title/subtitle forwarding when the scaffold does not render them.
...
- Notification private-message session account metadata use `PrivateSessionAccountInfo`
  ...
  and do not reintroduce `PrivateMessageAccountInfo` or its copy-only mapper. Keep
  `PrivateSession` and `PrivateSessionAccountInfo` hand-written in
...
- Notification private-message entities are owned by `private_message.dart` and
  ...
  `PrivateMessageEmoji.copyWith` helpers are archived, as is the unused
...
- Notification private-message API DTOs are owned by `private_message_model.dart`. Do
  ...
  list/map equality, and DTO-local `contentMap` parsing. DTO-local `copyWith` helpers
  and unused response-shell `toJson` helpers are archived; keep nested session/detail
  serializers because cache round-trips still use them.
...
- Notification chat send state keeps only behaviorful inputs. Preserve
  image-picker bytes and filename as a named record flowing from `ChatInput` into
  `Chat.sendImage`; do not reintroduce `ChatImageAttachment`, `ChatSendResult`, or
  `ChatSendStatus` wrapper types.
...
- History page list state stays page-owned. `HistoryPage` keeps its private
  ...
  restore a shared `history_controller.dart` provider wrapper for one page.
...
- Live room detail responses stay hand-written in `live_room_detail_model.dart`. Do not
  ...
  `mapEquals`; do not restore file-local `_mapEquals` or `_mapHash` clones for the
  room-detail metadata maps.
...
- Profile optional enrichment counters follow the shared scalar rule: relation
  ...
  `_parseLikes`, or `_parseVideoCount` wrappers.
...
- Notification chat send/upload response parsing is repository-owned. Do not reintroduce
  ...
  `NotificationRepositoryImpl.sendPrivateMessage` and `uploadImage`. Keep
...
- `RequestExecutor`, `Result`, and `AppError` stay the network and error boundary until a
  full replacement is planned and applied everywhere.
  Keep Dio bad-response classification in `AppError._fromDioException`; do not restore
  a one-use `_fromBadResponse` helper that only splits 401/403 from server responses.
...
- Endpoint policy resolver construction is owned by `NetworkQualityInterceptor`; do not
  ...
  only consumer.
- Resource API provider construction is owned by `ResourceApi`; keep
  `basicResourceApiProvider` and `resourceApiProvider` beside the Retrofit interface
  instead of restoring `resource_api_provider.dart` as a generated wrapper.
- WBI signing is owned by `WbiHelper`; do not keep a separate public `WbiSigner` class
  or `wbi_signer.dart` file when no caller consumes that boundary independently.
...
- Video detail info section renderers are archived when they are only consumed by
  `VideoInfoView`; keep uploader/follow row, stats, description/tags, action row, parts
  row, recommendation item, and collection summary private to `video_info_view.dart`, and
  keep one-use tag chips inline with the expandable description owner.
- Video player overlay chrome wrappers are archived when they are only consumed by
  ...
  one-use bottom-bar and lock-button glue inline in the overlay owner while leaving
  behaviorful progress/control widgets standalone.
...
- Video detail single-consumer interaction helpers are archived when they only apply
  ...
  logic private beside the owning controller instead of in a separate application file.
...
- Video detail conversion keeps single-use detail/page/request-user mapping private to
  `LoadVideoDetailWorkflow`, the only owner that turns the API detail payload into
  `VideoDetailViewData`. Do not reintroduce public `VideoDetailViewDataMapper`,
  `VideoPartViewDataMapper`, `VideoRequestUserStateMapper`, `toVideoDetailViewData`,
  `toVideoPartViewData`, or `toVideoRequestUserState` extension APIs for these one-use
  mappings.
...
- Favorite folder row renderers are archived when they are only consumed by
  `FavFolderList`; keep folder-list visual rows private to the list owner.
- Favorite resource row renderers are archived when they are only consumed by
  `FavoriteDetailPage`; keep favorite-detail resource rows private to that page owner.
...
- Favorites page-local tab/body and action wrappers are archived when they only serve
  ...
  with the page-owned tab controller and create-folder dialog callback.
...
- Notification chat message subwidgets are archived when they are only consumed by
  ...
  that only forward `BilibiliEmojiText` parameters, and keep behaviorful image/system/bubble
  renderers private to the owner.
- Notification message-support helper files are archived when they are only consumed by
  ...
  and system notification talker/text/URI parsing private to the feed-sync owner unless
  another repository owner consumes them.
...
- Notification private-session item renderers are archived when they are only consumed by
  ...
  private-session row.
- Notification feed item chrome is archived when it only serves `NotificationItemWidget`;
  keep the avatar tap target, header row, source preview, and failure feedback inline in
  the item owner instead of splitting them into one-use helper widgets. Do not restore
  `_getActionText` or `_getSourceText` when those strings only feed the same row build.
...
- Notification list page wrappers are archived when they only serve
  ...
  page-owned load gate and item builder callbacks.
- Search filter bars are archived when they are only consumed by `SearchResultView`; keep
  tab-local search filter UI private to the result view owner.
...
- Settings bottom-sheet selection widgets are archived when they are only consumed by
  `SettingsPage`; keep selection sheet/item UI private to the page owner.
...
- Search app bars are archived when they are only consumed by `SearchPage`; keep
  page-local app-bar composition private to the page owner.
...
- Search result item renderers are archived when they are only consumed by
  ...
  in one owner file so result handling and prefetch policy do not drift.
...
- Search query parameters are application-owned and hand-written. Keep `SearchQuery`
  ...
  should build the next query explicitly at the owning result view boundary.
...
- `AppNetworkImage` loading fallback geometry is owned by the loading branch in
  ...
  `width`/`height`, and `_buildDecoration(colorScheme)` inline there; do not restore a
  one-call `_buildLoadingPlaceholder` helper.
...
- Search page hook state is page-local widget lifecycle state owned by `SearchPage`.
  Do not reintroduce `use_search_view_model.dart`, public `useSearchViewModel`, public
  `SearchPageMode`, or a one-consumer `SearchPageState` wrapper class.
- Cross-feature contracts live in `core/contracts`. Shared visual components live in
  `shared/ui` after migration. Feature-specific assemblies belong to their owning feature.
- `part` files are acceptable for generated code. Hand-written `part` splits must be
  ...
  Live room content wrappers are archived when they only wrap one title, error, or
  decorative gradient branch for `LiveRoomContent` / `_DanmakuSection`.
  ...
  `_VideoListCardMedia.build` so overlay policy stays beside the stack it modifies.
  Feed-card reason tag wrappers are archived when they only render the single
  recommendation label overlay for `VideoCard`; keep semantic overlay colors, border,
  padding, and text style inline with `_VideoCardThumbnail.build`.
  Feed-card text/footer wrappers are archived when they only wrap `VideoCard` title,
  description, or author-row text; keep max-lines, overflow, typography, colors, and
  the more-action icon inline with `_VideoCardContent.build`.
  ...
  branches inline with `VideoThumbnail.build`, while the repeated stat item primitive may
  remain private.
  ...
  Profile app bar, notification item, and favorite detail pure UI handwritten splits are
  archived when they only contain private UI for one owning widget/page.
  ...
  `_FavResourceItem` must stay row-inline with `_FavoriteResourceRow`.
  Favorite picker selection/delta helpers are archived when only the picker sheet uses
  them; keep the folder-delta type and selected-id diff private to
  `video_favorite_picker_sheet.dart`.
  Profile page/header handwritten content/section splits are archived when they only
  contain private UI for one owning page/widget.
  ...
  owner, and stats rendering private to the page owner. One-use list shells stay inline
  with `_ToViewBody`.
  ...
  contain private UI for that page.
  ...
  contain private theme construction helpers for that single owner.
  ...
  preview render/decode helpers for the shared preview owner. Keep the public
  `AppImagePreview` constructor and `open(...)` API stable unless handled as a
  separate high-impact change.
  ...
  contain the paging content layer and private load helpers for that owner. Keep the
  one-use refresh result wrapper inline in `SmartPagingView.build` so refresh failure
  mapping stays at the EasyRefresh boundary.
  `SmartPagingContent` remains private unless a live caller needs EasyRefresh-free paging
  fallback rendering directly.
  ...
  single private editor widget for that page owner.
  ...
  Live danmaku list view wrappers are archived when they are only consumed by
  `LiveRoomContent`; keep the renderer private to the content owner.
  Dynamic comments empty-state UI is private when only `DynamicCommentsSliver` renders
  it.
  ...
  Video detail info header/engagement splits are archived when they only contain private
  render sections for `VideoInfoView`.
  ...
  row UI for `PlayerSettingsSheet`.
  Player settings option-section splits are archived when they only contain private
  speed/quality option UI for `PlayerSettingsSheet`.
  Vertical video top/right and bottom-bar control splits are archived when they only
  contain private render helpers for `VerticalVideoPage`; one-use right action rows stay
  inline with `_RightBar`.
  ...
  owner consumes the scan limits and logical image sizes.
  ...
  private render helpers for the single comment item owner.
  ...
  `favorite_mapper.dart` file when `FavRepositoryImpl` is the only consumer.
  ...
  Live header viewer-avatar helpers are archived when the only stack builder can own the
  avatar container inline beside the owning `LiveHeader`.
...
- Listen sleep timer state is owned by `listen_sleep_timer_view_model.dart`. Do not
  ...
  a one-use `_PresetMinuteTextButton` wrapper.
...
- Audio playback snapshot state is owned by `audio_playback_state_gate.dart`. Do not
  ...
  service-owned. Do not restore the unused `AudioPlaybackSnapshot.copyWith` helper.
...
- Article detail state is owned by `article_detail_view_model.dart`. Do not restore
  ...
  and submit-state behavior remain controller-owned.
- Danmaku option state is owned by `danmaku_option.dart`. Do not restore
  ...
  The value is reconstructed from `DanmakuSettings` at the call site; do not restore an
  unused `copyWith` helper.
...
- Notification chat state is owned by `chat_view_model.dart`. Do not restore
  ...
  `mapEquals` and inline unordered hashing; do not restore a local `_mapHash` helper.
...
- Article detail parsing must use shared JSON scalar utilities for integer coercion.
  ...
  `extractInitialState` JSON decode/type normalization inline in `ArticleDetailParser`;
  do not restore a one-use `_parseInitialState` helper.
