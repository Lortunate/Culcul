...
void main() {
  test(
    ...
    () async {
      ...
      final initialItems = await container.read(homePopularProvider.future);
      expect(initialItems.first.title, 'cached');
      await Future<void>.delayed(const Duration(milliseconds: 10));
      expect(dataSource.popularForceRefreshCalls, 1);

      dataSource.completePopularRefresh();
      await Future<void>.delayed(const Duration(milliseconds: 10));

      final refreshedItems = container.read(homePopularProvider).requireValue;
  ...
  test('home popular skips silent refresh when first page cache is absent', () async {
    ...
    expect(dataSource.popularForceRefreshCalls, 0);
  });
}
...
class _FakeHomeRepositoryImpl extends HomeRepositoryImpl {
  ...
  Future<Result<List<VideoModel>, AppError>> fetchRecommendPage({
    ...
    return Success(<VideoModel>[_video(title: 'recommend')]);
  }
  ...
  Future<Result<List<VideoModel>, AppError>> fetchPopularPage({
    ...
    if (forceRefresh) {
      popularForceRefreshCalls++;
      return _popularRefreshCompleter.future;
    }
    return Success(<VideoModel>[_video(title: 'cached')]);
  }

  void completePopularRefresh() {
    if (_popularRefreshCompleter.isCompleted) {
      return;
    }
    _popularRefreshCompleter.complete(Success(<VideoModel>[_video(title: 'fresh')]));
  }
}
